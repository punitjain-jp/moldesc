...
This app allows you to calculate Descriptors of Molecules (aka **Molecular Descriptors**) whic is used for computational drug discovery projects such as for the construction of quantitative structure-activity/property relationship (QSAR/QSPR) models.
...
with st.sidebar.header('1. Upload your CSV data'):
    ...
    st.sidebar.markdown("""
[Example CSV Input File](https://github.com/punitjain-jp/moldesc/raw/main/acetylcholinesterase_bioactivity_data_pIC50.csv)
""")
...
with st.sidebar.header('3. Set parameters'):
    ...
    df0 = pd.read_csv('acetylcholinesterase_bioactivity_data_pIC50.csv')
    all_mol = df0.shape[0]
...
if uploaded_file is not None:
    ...
    if st.button('Press to use Example Dataset'):
        ...
        def load_data():
            ...
            df = pd.read_csv('acetylcholinesterase_bioactivity_data_pIC50.csv').iloc[:number2calc,1:]
            return df
        df = load_data()
        df2 = pd.concat([df[name_smiles], df[name_mol]], axis=1)
        # Write CSV data
